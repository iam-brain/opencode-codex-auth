...
Pull request CI stays intentionally lean. After an exact-head review, maintainers manually run clean-room verify, tarball smoke, Windows smoke, and secret scanning against that reviewed branch and SHA; fork pull requests run those checks automatically because their branches cannot be dispatched from this repository. Dependency vulnerability auditing via `npm audit` runs on default-branch pushes instead of every PR.
