...
PR GitHub CI is intentionally slimmer than local `verify`: it keeps the clean-room Ubuntu verify job, Linux tarball smoke, Windows smoke, and secret scanning. For same-repository PRs, a maintainer runs `ci.yml` with the PR branch plus its reviewed `base_sha` and `head_sha`, then runs `secret-scan.yml` with that branch and `head_sha`; both workflows reject a dispatch if the branch no longer resolves to the reviewed head. Fork pull requests run these checks automatically because their branches cannot be selected by a workflow dispatch in this repository. The separate `npm audit` dependency audit remains GitHub-hosted, but it now runs on default-branch pushes instead of every PR.
