...
1. `config` records each custom agent's OpenCode mode. `chat.params` resolves session lineage through OpenCode's session API and classifies the execution as root, child, or auxiliary. Session lineage is authoritative for `mode: all`; built-in and configured modes provide a fail-closed fallback.
