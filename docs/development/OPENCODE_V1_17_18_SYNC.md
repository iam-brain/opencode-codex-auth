...
5. **OpenCode does not implement a special GPT-5.6 Ultra wire effort.** At v1.17.18 it accepts future post-5.4 model IDs through the dynamic filter, while reasoning variants come from provider/catalog capabilities. Therefore this repo's logical `ultra` policy and its default wire `max` effort remain a default-off WIP extension, not an OpenCode-native parity item; keep it behind `runtime.ultra`, catalog-gated, and keep native mode free of the Ultra delegation overlay. `runtime.ultraReasoningEffort` may explicitly lower the wire effort without changing the logical delegation policy.
