...
const ORCHESTRATOR_FRONTMATTER = `---
...
function stripLeadingFrontmatter(content: string): string {
  ...
  if (closing < 0) return content.trim()
...
async function ensureTemplateFile(
  ...
  const template = await resolveOrchestratorAgentTemplate(cacheDir)
  if (existingContent === template) {
    return { created: false, updated: false }
  }

  await fs.mkdir(path.dirname(filePath), { recursive: true })
  await fs.writeFile(filePath, template, { encoding: "utf8", mode: 0o600 })
...
export async function installOrchestratorAgent(
  ...
  const ensured = await ensureTemplateFile(filePath, input.force === true, input.cacheDir)
