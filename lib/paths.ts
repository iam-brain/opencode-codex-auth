...
export const LEGACY_AUTH_FILE = "auth.json"

export function defaultAuthPath(): string {
  return path.join(os.homedir(), ".config", "opencode", CODEX_ACCOUNTS_FILE)
}

export function legacyAuthPathFor(filePath: string): string {
  return path.join(path.dirname(filePath), LEGACY_AUTH_FILE)
}

export function opencodeProviderAuthPath(): string {
  return path.join(os.homedir(), ".local", "share", "opencode", LEGACY_AUTH_FILE)
}
