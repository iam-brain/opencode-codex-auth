  ...
  it("remaps non-permissions developer messages to user in codex mode by default", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        outboundRoles = Array.isArray(body.input)
          ? body.input.map((item) => (typeof item?.role === "string" ? item.role : ""))
          : []
        return new Response("ok", { status: 200 })
      })
    )
    ...
    expect(afterTransformCall?.[2]?.developerMessagesRemapped).toBe(true)
    expect(afterTransformCall?.[2]?.developerMessageRemapReason).toBe("updated")
    expect(afterTransformCall?.[2]?.developerMessageRemapCount).toBe(1)
    expect(afterTransformCall?.[2]?.developerMessagePreservedCount).toBe(1)

    expect(outboundRoles).toEqual(["user", "developer", "user"])
  })
})
